                    ...
                    <a-col>
                        ...
                        <a-modal centered="true" v-model:visible="visible" title="添加习题" @ok="createQuestion" :width="1000" ok-text="确认" cancel-text="取消">
                            <a-row type="flex" :gutter="[10,20]" id="flexbox" class="my-scroll">
                                ...
                                <a-col :span="21">
                                    <a-select ref="select" v-model:value="newQuestion.questionType" @focus="focus" @change="handleChange" style="width:200px">
    ...
    setup() {
        ...
        watch(
            ...
            (val) => {
                ...
                state.indeterminate =
                    !!val.length &&
                    val.length < metaPaper.value.questions.length;
                state.checkAll =
                    val.length === metaPaper.value.questions.length;
            }
        );
        ...
        const deleteQuestion = () => {
            for (let index of state.checkedList) {
                let question = metaPaper.value.questions;
                metaPaper.value.questions.splice(index, 1);
                metaPaper.value.questionNumber--;
                metaPaper.value.totalScore -=  question[index].score;
            }
        };
