<template>
    <a-layout style="min-height: 100vh">
        <a-layout-header class="my-header">
            <a-typography-text strong="true" style="font-size:17px">{{questionData.publishName}}</a-typography-text>
        </a-layout-header>

        <a-statistic-countdown v-if="this.questionData!=null && this.questionData.finishStatus!=2" title="剩余时间" :value="deadline" class="my-countdown" @finish="submit" />
		...
		getDetail() {
			...
			paperApi
				...
				.then(result => {
					...
					let now = Date.now();
					let passTime = (now - moment(this.questionData.actualStartTime).valueOf()) / 1000;
					this.deadline = (this.questionData.totalTime - passTime) * 1000 + Date.now();
